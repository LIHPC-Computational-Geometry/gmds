  ...
  namespace hybrid
  {
    ...
    namespace operators
    {
      ...
      public:
        ...
        DelaunayPointInsertion(SimplexMesh* simplexMesh, const simplicesNode::SimplicesNode& simpliceNode, const CriterionRAIS& criterion, std::vector<TSimplexID>& initialCavity, bool& status, const gmds::BitVector& markedNodes, std::vector<TSimplexID>& deletedSimplex, const std::multimap<TInt, TInt>& facesAlreadyBuilt, std::vector<TSimplexID> markedSimplex = std::vector<TSimplexID>{}, bool UseDelaunay = true);
