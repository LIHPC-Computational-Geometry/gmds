...
/*----------------------------------------------------------------------------*/
template<> void Mesh::negateMaskMark<Node>(const TInt AMarkNumber)
{
	m_maskMarks_nodes.flip(AMarkNumber);
}
/*----------------------------------------------------------------------------*/
template<> void Mesh::negateMaskMark<Edge>(const TInt AMarkNumber)
{
	m_maskMarks_edges.flip(AMarkNumber);
}
/*----------------------------------------------------------------------------*/
template<> void Mesh::negateMaskMark<Face>(const TInt AMarkNumber)
{
	m_maskMarks_faces.flip(AMarkNumber);
}
/*----------------------------------------------------------------------------*/
template<> void Mesh::negateMaskMark<Region>(const TInt AMarkNumber)
{
	m_maskMarks_regions.flip(AMarkNumber);
}
/*----------------------------------------------------------------------------*/
template<> bool Mesh::isMarked<Node>(const TCellID& ACellID, int AMarkNumber) const
{
	return (*m_marks[0])[ACellID][AMarkNumber]!=m_maskMarks_nodes[AMarkNumber];
}
/*----------------------------------------------------------------------------*/
bool Mesh::isMarked(const Node& ACell, int AMarkNumber) const
{
...
/*----------------------------------------------------------------------------*/
bool Mesh::isMarked(const Edge& ACell, int AMarkNumber) const
{
...
/*----------------------------------------------------------------------------*/
bool Mesh::isMarked(const Face& ACell, int AMarkNumber) const
{
...
/*----------------------------------------------------------------------------*/
bool Mesh::isMarked(const Region& ACell, int AMarkNumber) const
{
...
/*----------------------------------------------------------------------------*/
void Mesh::markTo(const Node& ACell, int AMarkNumber, bool AState)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::markTo(const Edge& ACell, int AMarkNumber, bool AState)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::markTo(const Face& ACell, int AMarkNumber, bool AState)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::markTo(const Region& ACell, int AMarkNumber, bool AState)
{
	markTo<Region>(ACell.id(), AMarkNumber, AState);
}
/*----------------------------------------------------------------------------*/
void Mesh::mark(const Node& ACell, int AMarkNumber)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::mark(const Edge& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,true);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
void Mesh::mark(const Face& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,true);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
void Mesh::mark(const Region& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,true);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
void Mesh::unmark(const Node& ACell, int AMarkNumber)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::unmark(const Edge& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,false);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
void Mesh::unmark(const Face& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,false);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
void Mesh::unmark(const Region& ACell, int AMarkNumber)
{
	markTo(ACell,AMarkNumber,false);
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
template<> void Mesh::unmarkAll<Node>(const TInt AMarkNumber)
{
	for(auto id: nodes()){
...
/*----------------------------------------------------------------------------*/
template<> void Mesh::unmarkAll<Edge>(const TInt AMarkNumber)
{
	for(auto id:edges())
		unmark<Edge>(id,AMarkNumber);
}
/*----------------------------------------------------------------------------*/
template<> void Mesh::unmarkAll<Face>(const TInt AMarkNumber)
{
    for(auto id: faces()){
...
/*----------------------------------------------------------------------------*/
template<> void Mesh::unmarkAll<Region>(const TInt AMarkNumber)
{
    for(auto id: regions()){
...
/*----------------------------------------------------------------------------*/
void Mesh::changeModel(const MeshModel& AModel, const bool& ACallDoctor)
{
	if(ACallDoctor) {
...
/*----------------------------------------------------------------------------*/
void Mesh::changeModelWithoutDoctor(const MeshModel& AModel)
{
...
/*----------------------------------------------------------------------------*/
void Mesh::changeModelWithDoctor(const MeshModel& AModel)
{
//	MeshModel model_old = m_model;
//	MeshModel model_new = AModel;
//
...
/*----------------------------------------------------------------------------*/
Node Mesh::newNode(const TCoord& AX, const TCoord& AY, const TCoord AZ)
{
...
/*----------------------------------------------------------------------------*/
Node Mesh::newNode(const math::Point& APnt)
{
	return newNode(APnt.X(),APnt.Y(),APnt.Z());
}
/*----------------------------------------------------------------------------*/
Edge Mesh::newEdge(const Node& AN1, const Node& AN2)
{
	return newEdge(AN1.id(), AN2.id());
}
/*----------------------------------------------------------------------------*/
Edge Mesh::newEdge(const TCellID& AN1, const TCellID& AN2)
{
...
/*----------------------------------------------------------------------------*/
Face Mesh::newTriangle(const Node& AN1, const Node& AN2, const Node& AN3)
{
...
/*----------------------------------------------------------------------------*/
Region Mesh::newTet(const Node& AN1, const Node& AN2, const Node& AN3,
		const Node& AN4)
{
	return newTet(AN1.id(), AN2.id(), AN3.id(), AN4.id());
}
/*----------------------------------------------------------------------------*/
Region Mesh::newTet(const TCellID& AN1, const TCellID& AN2,
		const TCellID& AN3, const TCellID& AN4)
{
...
/*----------------------------------------------------------------------------*/
Region Mesh::newPyramid(const Node& AN1, const Node& AN2, const Node& AN3,
		const Node& AN4, const Node& AN5)
{
	return newPyramid(AN1.id(), AN2.id(), AN3.id(), AN4.id(), AN5.id());
}
/*----------------------------------------------------------------------------*/
Region Mesh::newPyramid(const TCellID& AN1, const TCellID& AN2,
		const TCellID& AN3, const TCellID& AN4,
		const TCellID& AN5)
{
...
/*----------------------------------------------------------------------------*/
Region Mesh::newPrism3(const Node& AN1, const Node& AN2, const Node& AN3,
		const Node& AN4, const Node& AN5, const Node& AN6)
{
	return newPrism3(
            AN1.id(), AN2.id(), AN3.id(),
            AN4.id(), AN5.id(), AN6.id());
}
/*----------------------------------------------------------------------------*/
Region Mesh::newPrism3(const TCellID& AN1, const TCellID& AN2,
		const TCellID& AN3, const TCellID& AN4,
...
{
	return newHex(AN1.id(), AN2.id(), AN3.id(), AN4.id(),
...
/*----------------------------------------------------------------------------*/
TCellID Mesh::getMaxLocalID(const TInt& ADim) const {
	TInt max_id=0;
	if(ADim==0)
...
/*----------------------------------------------------------------------------*/
void Mesh::deleteVariable(ECellType AType, const std::string& AName)
{
	switch(AType){
	case GMDS_NODE:
	...
	case GMDS_REGION:
		...
		break;
	default:
		throw GMDSException("Unmanaged type of value -> impossible to delete a variable");
	}
}
/*----------------------------------------------------------------------------*/
void Mesh::deleteVariable(ECellType AType, VariableItf* AVar)
{
	switch(AType){
	case GMDS_NODE:
...
/*----------------------------------------------------------------------------*/
std::vector<VariableItf*> Mesh::getAllVariables(ECellType AType) const
{
	switch (AType){
	case GMDS_NODE:{
...
/*----------------------------------------------------------------------------*/
bool Mesh::hasVariable(ECellType AType, const std::string &AName) const
{
	switch(AType){
	case GMDS_NODE:{
...
/*----------------------------------------------------------------------------*/
void Mesh::clearAndResizeNodeIDContainer(const TInt AMaxID)
{
	m_nodes_container->clear();
...
/*----------------------------------------------------------------------------*/
void Mesh::clearAndResizeEdgeIDContainer(const TInt AMaxID)
{
	m_edges_container->clear();
...
/*----------------------------------------------------------------------------*/
void Mesh::clearAndResizeFaceIDContainer(const TInt AMaxID)
{
	m_faces_container->clear();
...
/*----------------------------------------------------------------------------*/
void Mesh::clearAndResizeRegionIDContainer(const TInt AMaxID)
{
	m_regions_container->clear();
...
/*----------------------------------------------------------------------------*/
Node Mesh::newNodeWithID(const TCoord& AX, const TCoord& AY,
		const TCoord& AZ,
		const TCellID& AGID)
{
...
/*----------------------------------------------------------------------------*/
Region Mesh::newPyramidWithID(const TCellID& AN1, const TCellID& AN2,
		   const TCellID& AN3, const TCellID& AN4,
...
/*----------------------------------------------------------------------------*/
void Mesh::initializeGeometryClassification()
{
	classification[0] =
	  newVariable<cad::GeomEntity*, GMDS_NODE>("NodeClassification");
	classification[1] =
	  newVariable<cad::GeomEntity*, GMDS_EDGE>("EdgeClassification");
	classification[2] =
	  newVariable<cad::GeomEntity*, GMDS_FACE>("FaceClassification");
	classification[3] =
	  newVariable<cad::GeomEntity*, GMDS_REGION>("RegionClassification");
...
/*----------------------------------------------------------------------------*/
bool Mesh::doesGeometricClassificationExist(const int ADim){
...
/*----------------------------------------------------------------------------*/
Variable<cad::GeomEntity* >* Mesh::
getGeometricClassification(const int ADim)
{
	return classification[ADim];
}
/*----------------------------------------------------------------------------*/
cad::GeomEntity* Mesh::getGeometricClassification(const Node ACell)
{
	return (*(classification[0]))[ACell.id()];
}
/*----------------------------------------------------------------------------*/
cad::GeomEntity* Mesh::getGeometricClassification(const Edge ACell)
{
	return (*(classification[1]))[ACell.id()];
}
/*----------------------------------------------------------------------------*/
cad::GeomEntity* Mesh::getGeometricClassification(const Face ACell)
{
	return (*(classification[2]))[ACell.id()];
}
/*----------------------------------------------------------------------------*/
cad::GeomEntity* Mesh::getGeometricClassification(const Region ACell)
{
	return (*(classification[3]))[ACell.id()];
}
/*----------------------------------------------------------------------------*/
void  Mesh::
setGeometricClassification(const Node ACell, cad::GeomEntity* AGE){
	(classification[0])->set(ACell.id(),AGE);
}
/*----------------------------------------------------------------------------*/
void  Mesh::
setGeometricClassification(const Edge ACell, cad::GeomEntity* AGE){
	(classification[1])->set(ACell.id(),AGE);
}
/*----------------------------------------------------------------------------*/
void  Mesh::
setGeometricClassification(const Face ACell, cad::GeomEntity* AGE){
	(classification[2])->set(ACell.id(),AGE);
}
/*----------------------------------------------------------------------------*/
void  Mesh::
setGeometricClassification(const Region ACell, cad::GeomEntity* AGE){
	(classification[3])->set(ACell.id(),AGE);
}
/*----------------------------------------------------------------------------*/
    template<>	CellGroup<Node>* Mesh::newGroup<Node>(const std::string& AName)
    {
        std::list<CellGroup<Node>* >::iterator it = m_clouds.begin();
        ...
        CellGroup<Node>* c= new CellGroup<Node>(this,AName) ;
...
/*----------------------------------------------------------------------------*/
    template<> CellGroup<Node>* Mesh::getGroup<Node>(const unsigned int AIndex)
    {
...
/*----------------------------------------------------------------------------*/
    ...
    {
        std::list<CellGroup<Edge>* >::iterator it = m_lines.begin();
        ...
        CellGroup<Edge>* c = new CellGroup<Edge>(this,AName);
        m_lines.push_back(c);
        return m_lines.back();
    }
/*----------------------------------------------------------------------------*/
    template<> void Mesh::deleteGroup<Edge>(CellGroup<Edge>* AG){
        std::list<CellGroup<Edge>* >::iterator it = m_lines.begin();
        for(;it!=m_lines.end();it++)
            if(AG==*it){
                m_lines.erase(it);
...
/*----------------------------------------------------------------------------*/
    template<> CellGroup<Edge>* Mesh::getGroup<Edge>(const unsigned int AIndex)
    {
...
/*----------------------------------------------------------------------------*/
    ...
    {
        std::list<CellGroup<Face>* >::iterator it = m_surfaces.begin();
        ...
        CellGroup<Face>* c = new CellGroup<Face>(this,AName);
        m_surfaces.push_back(c);
        return m_surfaces.back();
    }
/*----------------------------------------------------------------------------*/
    template<> void Mesh::deleteGroup<Face>(CellGroup<Face>* AG){
        std::list<CellGroup<Face>* >::iterator it = m_surfaces.begin();
        for(;it!=m_surfaces.end();it++)
            if(AG==*it){
                m_surfaces.erase(it);
...
/*----------------------------------------------------------------------------*/
    template<> CellGroup<Face>* Mesh::getGroup<Face>(const unsigned int AIndex)
    {
...
/*----------------------------------------------------------------------------*/
    ...
    {
        std::list<CellGroup<Region>* >::iterator it = m_volumes.begin();
        ...
        CellGroup<Region>* c = new CellGroup<Region>(this,AName);
        m_volumes.push_back(c);
        return m_volumes.back();
    }
/*----------------------------------------------------------------------------*/
    template<> void Mesh::deleteGroup<Region>(CellGroup<Region>* AG){
        std::list<CellGroup<Region>* >::iterator it = m_volumes.begin();
        for(;it!=m_volumes.end();it++)
            if(AG==*it){
                m_volumes.erase(it);
...
/*----------------------------------------------------------------------------*/
    template<> CellGroup<Region>* Mesh::getGroup<Region>(const unsigned int AIndex)
    {
...
/*----------------------------------------------------------------------------*/
   ...
   void Mesh::deleteNode(TCellID n){
    ...
    void Mesh::deleteEdge(TCellID e){
    ...
    /*------------------------------------------------------------------------*/
void Mesh::serialize(std::ostream& AStr)
{
...
/*------------------------------------------------------------------------*/
void Mesh:: unserialize(std::istream& AStr) {
...
/*----------------------------------------------------------------------------*/
void Mesh::
getCommonNodes(const Face& AF1, const Face& AF2, std::vector<TCellID>& ANodes)
{
	ANodes.clear();
...
/*----------------------------------------------------------------------------*/
    void Mesh::
    getCommonFaces(const Node &AN1, const Node &AN2, std::vector<TCellID>& AVec) {
        AVec.clear();
        std::vector<TCellID> set1 = AN1.getIDs<Face>();
        std::vector<TCellID> set2 = AN2.getIDs<Face>();

        bool found;
        for(unsigned int i1 = 0; i1<set1.size();i1++)
        {
            found=false;
            for(unsigned int i2 = 0; i2<set2.size() && !found;i2++)
                if(set1[i1]==set2[i2])
                {
                    AVec.push_back(set1[i1]);
                    found=true;
                }
        }
    }
/*----------------------------------------------------------------------------*/
    std::vector<TCellID> Mesh::getCommonFaces(const Node &AN1, const Node &AN2) {
    std::vector<TCellID> ids;
