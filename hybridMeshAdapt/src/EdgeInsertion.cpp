...
EdgeInsertion<Criterion>::EdgeInsertion(SimplexMesh* simplexMesh, const simplicesNode::SimplicesNode& simpliceNodeA, const simplicesNode::SimplicesNode& simpliceNodeB)
{
  //todo
  if(simplexMesh != nullptr)
  {
    ...
    if(simpliceNodeB.shell(simpliceNodeA).size()  > 0)
    {
      if(std::find(initCavity.begin(), initCavity.end(), boundary) == initCavity.end())
      {
        ...
        CavityOperator::CavityIO&&  cavity = cavOp.cavity<Criterion>(initCavity, simpliceNodeA);
        std::vector<TInt>&               nodesInCav       = cavity.nodeInCavity();
        std::vector<std::vector<TInt>>&& pointsToConnect  = cavity.pointToConnect();


        for(auto const & simplexInCavity : cavity.cavityIn())
        {
