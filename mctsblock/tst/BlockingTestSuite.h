...
setUp(gmds::cad::FACManager &AGeomManager)
{
	...
	std::string vtk_file = dir + "/tet_in_box.vtk";//tet_in_box
...
/*----------------------------------------------------------------------------*/
std::tuple<int,int,int,int> get_node_statistics(gmds::mctsblock::Blocking& ABlocking){
    auto nb_on_vertex=0;
    auto nb_on_curve=0;
...
/*----------------------------------------------------------------------------*/
std::tuple<int,int,int> get_edge_statistics(gmds::mctsblock::Blocking& ABlocking){
    auto nb_on_curve=0;
    auto nb_on_surface=0;
    auto nb_in_volume=0;
    std::vector<gmds::mctsblock::Blocking::Edge> all_edges = ABlocking.get_all_edges();
...
/*----------------------------------------------------------------------------*/
std::tuple<int,int> get_face_statistics(gmds::mctsblock::Blocking& ABlocking){
    auto nb_on_surface=0;
    auto nb_in_volume=0;
    std::vector<gmds::mctsblock::Blocking::Face> all_faces = ABlocking.get_all_faces();
...
/*----------------------------------------------------------------------------*/
void export_vtk(gmds::mctsblock::Blocking& ABlocking, int AModel, const std::string& AFileName){
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, global_cell_accessors)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, remove_block)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, single_block)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, single_block_parallel_edges)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model);
	...
	for (auto it = bl.gmap()->attributes<1>().begin(), itend = bl.gmap()->attributes<1>().end(); it != itend; ++it) {
		std::vector<gmds::mctsblock::Blocking::Edge> parallel_edges;
		bl.get_all_sheet_edges(it,parallel_edges);
		ASSERT_EQ(4, parallel_edges.size());
	}

	std::vector<std::vector<gmds::mctsblock::Blocking::Edge> > all_edges= bl.get_all_sheet_edge_sets();
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, get_edges_of_a_block)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, split_one_block_twice)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model, true);
	gmds::mctsblock::BlockingClassifier cl(&bl);
	cl.classify();
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, cut_sheet_param_propag)
{
	...
	std::vector<gmds::mctsblock::Blocking::Face> all_faces = bl.get_all_faces();
	std::vector<gmds::mctsblock::Blocking::Face> surf;
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, init_from_geom_bounding_box)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model,true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, single_block_to_mesh)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, projection_point_to_edges)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_topological_queries)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model, true);
	std::vector<gmds::mctsblock::Blocking::Node> bl_nodes = bl.get_all_nodes();
	std::vector<gmds::mctsblock::Blocking::Edge> bl_edges = bl.get_all_edges();
	std::vector<gmds::mctsblock::Blocking::Face> bl_faces = bl.get_all_faces();

	for(auto n: bl_nodes){
		    std::vector<gmds::mctsblock::Blocking::Face> fs = bl.get_faces_of_node(n);
		    ASSERT_EQ(3, fs.size());
		    std::vector<gmds::mctsblock::Blocking::Edge> es = bl.get_edges_of_node(n);
		    ASSERT_EQ(3, es.size());
		    std::vector<gmds::mctsblock::Blocking::Block> bs = bl.get_blocks_of_node(n);
		    ASSERT_EQ(1, bs.size());
	}
	for(auto e: bl_edges){
		    std::vector<gmds::mctsblock::Blocking::Node> ns = bl.get_nodes_of_edge(e);
		    ASSERT_EQ(2, ns.size());
		    std::vector<gmds::mctsblock::Blocking::Face> fs = bl.get_faces_of_edge(e);
		    ASSERT_EQ(2, fs.size());
		    std::vector<gmds::mctsblock::Blocking::Block> bs = bl.get_blocks_of_edge(e);
		    ASSERT_EQ(1, bs.size());
	}
	for(auto f: bl_faces){
		    std::vector<gmds::mctsblock::Blocking::Node> ns = bl.get_nodes_of_face(f);
		    ASSERT_EQ(4, ns.size());
		    std::vector<gmds::mctsblock::Blocking::Edge> es = bl.get_edges_of_face(f);
		    ASSERT_EQ(4, es.size());
		    std::vector<gmds::mctsblock::Blocking::Block> bs = bl.get_blocks_of_face(f);
		    ASSERT_EQ(1, bs.size());
	}
}
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_init_from_ig_mesh)
{
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_chord_query)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, false);
    ...
    std::vector<gmds::mctsblock::Dart3> darts;
    std::vector<gmds::mctsblock::Blocking::Face> bl_faces = bl.get_all_faces();
    for(auto f:bl_faces){
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_chord_collapse)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, false);
    ...
    for(auto i=0; i<bl_faces.size() && !found_face;i++){
        gmds::mctsblock::Blocking::Face fi = bl_faces[i];
    ...
    std::vector<gmds::mctsblock::Blocking::Node> f_nodes = bl.get_nodes_of_face(bl_faces[face_id]);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_validate_pillow)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
    ...
    std::vector<std::vector<gmds::mctsblock::Blocking::Edge>> edges = bl.get_all_sheet_edge_sets();
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_2)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_3)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_4)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_5)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_6)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_7)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_8)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_9)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_10)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_11)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, test_pillow_12)
{
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model, true);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, save_vtk_blocking){
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model,true);
	gmds::mctsblock::BlockingClassifier classifier(&bl);

	bl.save_vtk_blocking("testSaveWork.vtk");
}
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, get_Id_block){
	gmds::cad::FACManager geom_model;
	setUp(geom_model);
	gmds::mctsblock::Blocking bl(&geom_model,true);
	gmds::mctsblock::BlockingClassifier classifier(&bl);
...
/*----------------------------------------------------------------------------*/
TEST(BlockingTestSuite, check_capt_Element){
    gmds::cad::FACManager geom_model;
    setUp(geom_model);
    gmds::mctsblock::Blocking bl(&geom_model,true);
    gmds::mctsblock::BlockingClassifier classifier(&bl);
