        ...
        public:
            ...
            vector_type getOneOrtho() const{
                ...
                if (TDim==2){
                    ...
                    for (auto i = 0;
                         i < dimension && first_non_zero_field == -1; i++) {
                        if (m_data[i] != 0)
                            first_non_zero_field = i;
                    }
             ...
             */
            void setX(const TCoord &AX) { m_data[0] = AX; }

            void setY(const TCoord &AY) { m_data[1] = AY; }

            void setZ(const TCoord &AZ) { m_data[2] = AZ; }
...
//                else
//                {
//                    throw GMDSException("Z() accessor is only available for ND vectors with N>2");
//                }
            }
             ...
             */
            VectorND(const double &AX, const double &AY)
            {
                m_data[0] = AX;
                m_data[1] = AY;
            }
             ...
             */
            VectorND(const double &AX, const double &AY, const double &AZ)
            {
                m_data[0] = AX;
                m_data[1] = AY;
                m_data[2] = AZ;
            ...
            /*-----------------------------------------------------------*/
            /** \brief Specific constructor from a point
             *
             * \param AP A point
             */
            VectorND(const Point &AP) {
                //if constexpr(TDim==3){
                    m_data[0] = AP.X();
                    m_data[1] = AP.Y();
                    m_data[2] = AP.Z();
                ...
                //else
                //    {
                 //   throw GMDSException("Building vectors from point is only possible for 3D vectors");
                //}
            }
             ...
             */
            VectorND(const Point &AP1, const Point &AP2) {
             ...
             */
            inline TCoord
            orientedAngle(const VectorND &AV, const VectorND &AOrtho = VectorND(0, 0, 1)) const {
             ...
             */
            inline TCoord
            angleIn02PI(const VectorND &AV, const VectorND &AOrtho = VectorND(0, 0, 1)) const {
                VectorND ref = cross(AOrtho);
             ...
             */
            bool isOrthogonal(const VectorND& AV) const {return (dot(AV)==0.0);}
        ...
        template <int TDim, typename TVec>
