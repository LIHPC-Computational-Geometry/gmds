...
{	std::vector<double>  hist_empty;
	MCTSStatePolycube *init_state = new MCTSStatePolycube(m_geom,m_blocking,hist_empty,"init");
	tree = new MCTSTree(init_state);}
/*----------------------------------------------------------------------------*/
MCTSAlgorithm::~MCTSAlgorithm(){;}
/*----------------------------------------------------------------------------*/
void MCTSAlgorithm::execute(std::string ANameGeo)
{
	std::cout<<"==========================================================="<<std::endl;
	...
	MCTSState *state = new MCTSStatePolycube(this->m_geom, this->m_blocking, std::vector<double> (),ANameGeo);
	//state->print();                           // IMPORTANT: state will be garbage after advance_tree()
	MCTSAgent agent(state,ANameGeo, 10000);
	do {
		agent.feedback();
		agent.genmove();
