...
{
	std::vector<TCellID> free_node_ids;
	for(auto n_id:m_mesh->nodes()){
		if(m_node_constrained->value(n_id)!=1){
			free_node_ids.push_back(n_id);
		}
	}

	for(auto n_id:free_node_ids){
		...
		m_node_to_stencil.insert(std::make_pair(n_id,current_stencil));
		std::cout<<"Node "<<n_id<<std::endl;
		for(auto i=0;i<3;i++){
			for(auto j=0;j<3;j++){
				std::cout<<m_node_to_stencil[n_id].val[i][j]<<" ";
			}
			std::cout<<std::endl;
		}
	}
