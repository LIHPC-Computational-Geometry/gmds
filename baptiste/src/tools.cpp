...
void gmds::computeVolFrac(gmds::Mesh *AMesh, gmds::Mesh *AImprintMesh, gmds::Variable<double> *quadVolFrac, gmds::Variable<double> *targetVolFrac)
{
	// check validity of the inputs
	bool valid_input = true;
	std::string msg("volfraccomputation_2d ");
	...
	for(auto tri_id: AImprintMesh->faces()) {
		...
		for(auto f_id: AMesh->faces()) {
			...
			gmds::math::Point A(xyz[0][0], xyz[0][1], xyz[0][2]);
			gmds::math::Point B(xyz[1][0], xyz[1][1], xyz[1][2]);
			gmds::math::Point C(xyz[2][0], xyz[2][1], xyz[2][2]);
			gmds::math::Point D(xyz[3][0], xyz[3][1], xyz[3][2]);
			gmds::math::Quadrilateral q(A, B, C, D);
			...
			double vf = targetVolFrac->value(tri_id);
			double vfQuad = quadVolFrac->value(f_id);
			//			std::cout<<"f_id "<<f_id<<" volsurf "<<volsurf<<" vf "<<vf<<" om "<<om[0]/volsurf<<std::endl;
			// add but do not forget to divide by cell area
			targetVolFrac->set(tri_id, vf + om[0]/volsurf);
			quadVolFrac->set(f_id, vfQuad + om[0]/volsurfQuad);
		}
	}
}
