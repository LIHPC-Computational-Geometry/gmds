...
/*---------------------------------------------------------------------------*/
MCTSTree* MCTSTree::get_best_winrate_child() const {
	// sanity check
	if(!is_fully_expanded())
		return nullptr;

	float best_winrate_score = -std::numeric_limits<float>::max();
	MCTSTree* best_node = nullptr;

	// iterate all immediate children and find best winrate score
	int num_children = m_children.size();
	for(auto i = 0; i < num_children; i++) {
		auto child = m_children[i];
		float winrate_score = (float)child->cumulative_reward / (child->number_visits);

		if(winrate_score > best_winrate_score) {
			best_winrate_score = winrate_score;
			best_node = child;
		}
	}
