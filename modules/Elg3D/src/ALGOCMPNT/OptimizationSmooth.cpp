...
namespace elg3d {
    ...
    const double OptimizationSmooth_DELTA_OPTIM = 0.001;

    const int OptimizationSmooth_LINE_SEARCH_NBTRY = 16;

    /*----------------------------------------------------------------------------*/
    struct OptimizationSmooth_nonFixedNodesSelection
    {
        ...
        OptimizationSmooth_nonFixedNodesSelection(
...
//    };
    ...
    /*----------------------------------------------------------------------------*/
    struct OptimizationSmooth_smoothNode_optim_2D
    {
        const kmds::GrowingView<kmds::TCellID>* selection;
        kmds::Mesh* mesh;
        const kmds::Connectivity* c_N2C;
        ...
        OptimizationSmooth_smoothNode_optim_2D(
        ...
        operator()(int i) const {
            ...
            double edgelen_avg = 0.;

            for(int i_n=0; i_n<nids.size(); i_n++) {

                gmds::math::Point pt_n = mesh->getNodeLocation(nids(i_n));
                edgelen_avg += pt.distance(pt_n);
            }

            if(nids.size() != 0) {
                edgelen_avg /= nids.size();
            }

            // compute the gradient for the minimum scaled jacobian
            gmds::math::Point pt_deltax(pt.X() + edgelen_avg * OptimizationSmooth_DELTA_OPTIM, pt.Y() , pt.Z());
            gmds::math::Point pt_deltay(pt.X(), pt.Y() + edgelen_avg * OptimizationSmooth_DELTA_OPTIM , pt.Z());
            gmds::math::Point pt_deltaz(pt.X(), pt.Y() , pt.Z() + edgelen_avg * OptimizationSmooth_DELTA_OPTIM);

            double qual   = HUGE_VALF;
            double qual_x = HUGE_VALF;
            double qual_y = HUGE_VALF;
            double qual_z = HUGE_VALF;
//            double qual   = 0.;
//            double qual_x = 0.;
//            double qual_y = 0.;
//            double qual_z = 0.;
            ...
            for (int i_c = 0; i_c < cells.size(); i_c++) {
                ...
                double qual_tmp   = c.scaledJacobianAt(nid, pt);
                double qual_x_tmp = c.scaledJacobianAt(nid, pt_deltax);
                double qual_y_tmp = c.scaledJacobianAt(nid, pt_deltay);
//                double qual_z_tmp = c.scaledJacobianAt(nid, pt_deltaz);

                qual   = std::min(qual, qual_tmp);
                qual_x = std::min(qual_x, qual_x_tmp);
                qual_y = std::min(qual_y, qual_y_tmp);
//                qual_z = std::min(qual_z, qual_z_tmp);
//                qual   += qual_tmp;
//                qual_x += qual_x_tmp;
//                qual_y += qual_y_tmp;
//                qual_z += qual_z_tmp;
...
//            gmds::math::Vector grad(qual_x - qual, qual_y - qual, qual_z - qual);
            ...
            gmds::math::Point pt_new = pt + edgelen_avg * grad;

            if((*varNodeGeomAssociation)[nid] != reinterpret_cast<std::uintptr_t>(nullptr)) {
                reinterpret_cast<gmds::cad::GeomEntity *>((*varNodeGeomAssociation)[nid])->project(pt_new);
            }

            gmds::math::Point positions[OptimizationSmooth_LINE_SEARCH_NBTRY];
            positions[0] = pt;
            positions[OptimizationSmooth_LINE_SEARCH_NBTRY - 1] = pt_new;

            for(int ipt=1; ipt<OptimizationSmooth_LINE_SEARCH_NBTRY - 1; ipt++) {
                gmds::math::Point pt_tmp = (1. - ((double )ipt/ (double )(OptimizationSmooth_LINE_SEARCH_NBTRY - 1))) * pt + (double) ipt/ ((double)(OptimizationSmooth_LINE_SEARCH_NBTRY - 1)) * pt_new;
                if((*varNodeGeomAssociation)[nid] != reinterpret_cast<std::uintptr_t>(nullptr)) {
            ...
            kmds::TCoord scaledJacobians[OptimizationSmooth_LINE_SEARCH_NBTRY][cells.size()];

            for(int ipt=0; ipt<OptimizationSmooth_LINE_SEARCH_NBTRY; ipt++) {
                ...
                for (int i_c = 0; i_c < cells.size(); i_c++) {
                    ...
                    kmds::Face c = mesh->getFace(cid);
                    scaledJacobians[ipt][i_c] = c.scaledJacobian();
                    minJ_new = std::min(minJ_new, scaledJacobians[ipt][i_c]);
                }
            ...
            for(int i_c=0; i_c<cells.size(); i_c++) {
                (*varQuality)[cells[i_c]] = scaledJacobians[index][i_c];
            }
        }
    };
...
//    }
    ...
    /*----------------------------------------------------------------------------*/
//    struct Optimizationsmooth_smoothNode_3D
//    {
//        const kmds::GrowingView<kmds::TCellID>* selection;
//        kmds::Mesh* mesh;
//        const kmds::Connectivity* c_N2C;
...
//
//
//        Optimizationsmooth_smoothNode_3D(
//
//                const kmds::GrowingView<kmds::TCellID>* selection_,
//                kmds::Mesh* mesh_,
//                const kmds::Connectivity* c_N2C_,
...
//    };
    ...
    /*----------------------------------------------------------------------------*/
    struct OptimizationSmooth_smoothNode_optim_3D
    {
        const kmds::GrowingView<kmds::TCellID>* selection;
        kmds::Mesh* mesh;
        const kmds::Connectivity* c_N2C;
        ...
        OptimizationSmooth_smoothNode_optim_3D(
...
//                reinterpret_cast<gmds::geom::GeomEntity *>((*varNodeGeomInterface)[nid])->project(pt);
//            }
//
//            if((*varNodeGeomAssociation)[nid] != reinterpret_cast<std::uintptr_t>(nullptr)) {
//                reinterpret_cast<gmds::geom::GeomEntity *>((*varNodeGeomAssociation)[nid])->project(pt);
//            }
//
//            kmds::TCoord xyz_old[3];
//            mesh->getNodeLocation(nid, xyz_old[0], xyz_old[1], xyz_old[2]);
...
//    };
    ...
    /*----------------------------------------------------------------------------*/
    void
    optimizationSmooth_2D(const int nbIter,
                          kmds::Mesh* AMesh,
...
//        std::cout<<"smartLaplacian_2D preptime "<<timer.seconds()<<std::endl;
        ...
        Kokkos::parallel_for(nbNodes, OptimizationSmooth_nonFixedNodesSelection(&nodesIDs, varfixed, &nonFixedNodes));
...
//        std::cout<<"smartLaplacian_2D buildGraph_N_N2F2N "<<timer.seconds()<<std::endl;
        ...
        graph.buildColoring(optimizationSmooth_NBMAXCOLORS);
...
//        std::cout<<"smartLaplacian_2D buildColoring "<<timer.seconds()<<std::endl;
        ...
        for(int icolor=0; icolor<optimizationSmooth_NBMAXCOLORS; icolor++) {
...
//                                                                  varquality));
                                     ...
                                     OptimizationSmooth_smoothNode_optim_2D(nodesNonFixed_indepset,
                                                                            AMesh,
                                                                            c_N2F,
                                                                            c_N2N,
                                                                            AVarNodeGeomAssociation,
...
//        std::cout<<"smartLaplacian_2D after deletevariables"<<std::endl;
    ...
    /*----------------------------------------------------------------------------*/
    void
    optimizationSmooth_3D(const int nbIter,
                          kmds::Mesh* AMesh,
...
//                                     SmartLaplacian_vertex2Node(nodesNonFixed_indepset, &nonFixedNodes));
                ...
                Kokkos::parallel_for(nodesNonFixed_indepset->getNbElems(),
//                                     OptimizationSmooth_smoothNode_3D(nodesNonFixed_indepset,
//                                                                      AMesh,
//                                                                      c_N2R,
//                                                                      c_N2N,
//                                                                      AVarNodeGeomAssociation,
//                                                                      varquality));
                OptimizationSmooth_smoothNode_optim_3D(nodesNonFixed_indepset,
                                                   AMesh,
                                                   c_N2R,
                                                   c_N2N,
                                                   AVarNodeGeomAssociation,
